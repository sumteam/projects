  ...
  try {
    ...
    if (requestLimit <= maxApiLimit) {
      ...
      const response = await withTimeout(
          fetch(
              `https://api.binance.us/api/v3/klines?symbol=${symbol}&interval=${timeframe.binanceInterval}&limit=${requestLimit}`
          ),
      ...
      for (let i = 0; i < numRequests; i++) {
        ...
        let url = `https://api.binance.us/api/v3/klines?symbol=${symbol}&interval=${timeframe.binanceInterval}&limit=${currentLimit}`;
        if (endTime) {
          url += `&endTime=${endTime}`;
        }
